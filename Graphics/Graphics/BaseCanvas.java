...
public class BaseCanvas extends Canvas{
	...
	//任务相关
	private boolean 													isEnable;//画板开关
	private boolean 													isRunning;//图像刷新线程开关
	
	//FPS相关
	private long 														exceptFPS=60;//预期FPS
	private float 														fps=0;//当前FPS
	private volatile long 											startTime=0;//开始时间
	private volatile long 											endTime=0;//结束时间
	private long 														sleepTime=0;//帧控制睡眠时间
	private Object														timeLocker;//时间线程锁
	...
	//鼠标相关
	private boolean													isMouseListening=false;//鼠标监听器
	private int															mouseKey;//鼠标按下的按键
	private int															isMousePress=-1;//鼠标按下状态标记
	private int															lastMouseState=-1;//最后一次鼠标状态
	public static final int											MOUSE_LEFT_BUTTON=1;//左键
	public static final int 											MOUSE_WHEEL=2;//中键
	public static final int											MOUSE_RIGHT_BUTTON=3;//右键
	private int 															wheelMoveValue;//滚轮滚动方向
	private boolean													mouseVisable=true;//鼠标可视开关
	
	//监听事件
	public MouseActionListener									mouseActionListener;//鼠标动作监听器
	public BorderHitListener 										borderhitListener;//边框碰撞监听器
	public ElementHitListener									elementHitListener;//元素碰撞监听器
	ElementControl														elementControl;//元素添加监听器(自己用)
	...
	private MouseListener mouseListener=new MouseListener() {
		...
		public void mouseReleased(MouseEvent e) {
			...
			checkElementMouseEvent(e, MouseState.MouseReleased);
		}
		...
		public void mousePressed(MouseEvent e) {
			...
			checkElementMouseEvent(e, MouseState.MousePress);
		}
	...
	//每个设置了鼠标事件的元素遍历.
	/*
	 * 在容器发生鼠标事件时，检查点击的区域，锁定顶层级未被遮挡的控件
	 */
	private BaseElement pressLockElement;
	private void checkElementMouseEvent(MouseEvent e,MouseState state) {
		//按下的情况
		/*
		 * 按下之后，会锁定顶层的已经打开的元素
		 */
		if(state==MouseState.MousePress) {
			for(int i=totalElement.size()-1;i>=0;i--) {
				if(totalElement.get(i).isVisable()&&totalElement.get(i).isInElement(e.getX(), e.getY())) {
					pressLockElement=totalElement.get(i);
					pressLockElement.isPrese=true;
